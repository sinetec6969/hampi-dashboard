  ...
  return (
    <div className="adsb-page">
      <div className="adsb-header">
        <span className="adsb-header-title" style={{ color: '#e08a3c' }}>APRS 144.390</span>
        ...
        <div className="adsb-side">
          {selSt ? (
            ...
            <div className="adsb-list" style={{ display: 'flex', flexDirection: 'column' }}>
              ...
              <div style={{ flex: 1, overflowY: 'auto', borderTop: '1px solid #343c3d', fontSize: 11, padding: 4 }}>
                {packets.map((p, i) => (
                  <div key={i} style={{ padding: '2px 0', borderBottom: '1px solid #252b2c' }}>
                    <span style={{ color: '#e08a3c' }}>{p.from}</span>{' '}
                    <span style={{ color: '#9aa5a0' }}>{ago(p.ts)}</span>{' '}
