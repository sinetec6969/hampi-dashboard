  ...
  return (
    <div className="panel" style={{ flex: '2', overflowY: 'hidden', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
        ...
        {activeCC > 0 && <span style={{ marginLeft: 'auto', fontSize: '0.65rem', color: '#66716c' }}>CC{activeCC}</span>}
      </div>

      {/* Active call info */}
      <div style={{ marginBottom: 6, paddingBottom: 6, borderBottom: '1px solid #252b2c', fontSize: '0.72rem', minHeight: 38 }}>
        {activeSrc ? (
          <>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
              <span className="badge badge-blue">TS{activeTs + 1}</span>
              {activeDst > 0 && (
                <span style={{ color: '#9aa5a0' }}>
                  TG&nbsp;<span style={{ color: '#fff' }}>{activeDst}</span>
                </span>
              )}
              <span style={{ color: '#9aa5a0' }}>ID&nbsp;<span style={{ color: '#fff' }}>{activeSrc}</span></span>
            </div>
            <div style={{ marginTop: 3, color: '#35d07f', fontWeight: 'bold', fontSize: '0.78rem', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
              {callsignLine}
            </div>
          </>
        ) : (
          <span style={{ color: '#66716c', fontSize: '0.7rem' }}>No active call</span>
        )}
      </div>
      ...
      <div style={{ overflowY: 'auto', flex: 1 }}>
        {frames.map((f, i) => (
          <div className="dmr-row" key={i} style={{ opacity: 1 - i * 0.04 }}>
            ...
            {f.src_id > 0 && (
              <span style={{ fontSize: '0.65rem', color: '#9aa5a0' }}>
                {f.alias || f.src_id}
                {f.dst_id > 0 && <span style={{ color: '#66716c' }}>→TG{f.dst_id}{f.tg_name ? ` ${f.tg_name}` : ''}</span>}
              </span>
            )}
