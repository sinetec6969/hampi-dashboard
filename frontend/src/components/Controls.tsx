import { useEffect, useState } from 'react'

interface Props { onTune: (freq:number, gain:number)=>void }

export default function Controls({ onTune }: Props) {
  const [freq, setFreq] = useState(438800000)
  const [gain, setGain] = useState(49.6)
  const [msg, setMsg] = useState('')
  const [clients, setClients] = useState<Record<string,number>>({})

  useEffect(() => {
    fetch('/api/status').then(r=>r.json()).then(d => {
      setFreq(d.freq); setGain(d.gain); setClients(d.clients||{})
      onTune(d.freq, d.gain)
    })
  }, [])

  async function tune() {
    const r = await fetch(`/api/tune?freq=${freq}&gain=${gain}`, {method:'POST'})
    if (r.ok) { setMsg('Tuned ✓'); onTune(freq, gain); setTimeout(()=>setMsg(''),2000) }
    else setMsg('Error')
  }

  return (
    <div style={{display:'flex',alignItems:'center',gap:12,fontSize:'0.8rem'}}>
      <label>Freq (Hz):
        <input type="number" value={freq} onChange={e=>setFreq(Number(e.target.value))}
          style={{width:120,marginLeft:6}}/>
      </label>
      <label>Gain: {gain.toFixed(1)}dB
        <input type="range" min={0} max={50} step={0.1} value={gain}
          onChange={e=>setGain(Number(e.target.value))} style={{marginLeft:6,width:80}}/>
      </label>
      <button className="btn" onClick={tune}>Tune</button>
      {msg && <span style={{color:'#00ff88'}}>{msg}</span>}
      <span style={{color:'#555',fontSize:'0.7rem'}}>
        WF:{clients.waterfall||0} DMR:{clients.dmr||0} AUD:{clients.audio||0}
      </span>
    </div>
  )
}
